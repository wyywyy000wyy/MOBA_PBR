...
public class GpuSkinningInstGenerator
{
    ...
    public void generatePrefab(string savePath, string prefabFileName, string dataFileName, string parentFolder)
    {
        ...
        instance.anim_data = animData;
        //instance.textAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(Path.Combine(savePath, dataFileName));

        string prefabPath = Path.Combine(Defines.AssetBundleEntityPath, prefabFileName);
    ...
    public void generateMaterial(string savePath, string matFileName, string mainTexPath)
    {
        ...
        instMaterial.SetVector("_AnimationTexSize", new Vector4(animData.texWidth, animData.texHeight, 0));
