...
public class GpuSkinningInstTools : EditorWindow 
{
    ...
    public static readonly string DEFAULT_SAVE_PREFAB_NOISE_VERT_NAME = ".prefab";//"_NoiseVertPre.prefab";
    // 默认material文件名称后缀
	...
	private void OnGUI() 
	{
		...
		EditorGUILayout.LabelField(string.Format("输出Prefab:{0}", Path.Combine(Defines.AssetBundleEntityPath, savePrefabName)));
    ...
    private static void GetAnimClips(GameObject obj)
    {
        ...
        var files = Directory.GetFiles(assetFolder, "*.fbx", SearchOption.AllDirectories);
        foreach(var file in files)
        {
            if(file != null)
            Debug.LogFormat("file:{0}", file);
            GetAnimClipsPath(file);
        }
    }

    private static void GetAnimClipsPath(string assetPath)
    {
