...
{
    /// <summary>
    /// SECTR will ignore these transforms and their children
    /// </summary>
    [SerializeField] private List<Transform> ignoreTransforms = new List<Transform>(); 
	...
	public class Child
	{
		public void Init(GameObject gameObject, Renderer renderer, Light light, Terrain terrain, SECTR_Member member, bool dirShadowCaster, Vector3 shadowVec)
		{
			...
			if(SECTR_Modules.VIS)
			{
                ...
                terrainCastsShadows = this.terrain && terrain.shadowCastingMode == UnityEngine.Rendering.ShadowCastingMode.On && (terrain.lightmapIndex == -1 || liveLightmaps);
#else
...
#region Private Utility Methods
	...
	private void _AddChildren(Transform childTransform, bool dirShadowCaster, Vector3 shadowVec)
	{
        ...
        if (childTransform.gameObject.activeSelf &&
           (childTransform == transform || !allMemberTable.ContainsKey(childTransform)) && (ignoreTransforms == null || !ignoreTransforms.Contains(childTransform)))
            { 
			...
			if(bakedOnlyLights != null && 
				childLight && 
#if UNITY_2017_3_OR_NEWER
				childLight.bakingOutput.isBaked &&
#else
				childLight.isBaked &&
#endif
